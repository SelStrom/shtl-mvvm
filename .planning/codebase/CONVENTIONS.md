...
**Язык комментариев:**
- **Все комментарии в коде — только на английском.** Это касается `//`, `/* */`, `///`, runtime-сообщений в `throw`/`Assert.IsTrue`/`Debug.Log`, имён переменных и идентификаторов.
- На русском пишутся только: документация в `.planning/` и `.md`-файлах, commit-сообщения, описания в Inspector (если они адресованы конечному пользователю-дизайнеру).
